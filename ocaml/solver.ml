...
let rec main_loop2 post_solution a =
  if a.(81) == 81 then post_solution (print a) else
  let i = chains a in
  if i < 0 then post_solution (print a)
    ...
    if a.(i) land m <> 0 then
      try
        ...
        may_restrict a' i m;
        main_loop post_solution a'
      with Impossible -> ()
  done

and main_loop post_solution a =
  ...
  try
    main_loop2 post_solution a;
    real_blocks := old_bs; real_neighbours := old_n
  with exn -> real_blocks := old_bs; real_neighbours := old_n; raise exn

let all_bits = ((1 lsl 9) - 1)

let solve_grid post_solution clues =
